...
class Quadrant:
    ...
    def inclusion_quadrant(self,q2):
        """
        Vérifie que le quadrant de p1 (noté q1) est 
        inclus dans le quadrant de p2 (noté q2). Si 
        ce n'est pas le cas, alors p1 ne peut pas 
        être inclus dans p2.
        Renvoie True si le quadrant de p1 est inclus 
        dans le quadrant de p2.
        """

        for i in range(2):
            
            if (self.min_coordinates[i] < q2.min_coordinates[i]) or (self.max_coordinates[i] > q2.max_coordinates[i]):
                return(False)
